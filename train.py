...
y_disc = tf.concat(values=[y, y * 0],axis=0)
...
# input image pairs
#
x_real_pair = tf.concat(axis=3, values=[x_nearest, x])
x_fake_pair = tf.concat(axis=3, values=[x_nearest, gen])
...
xx = tf.concat(axis=0, values=[x_real_pair, x_fake_pair])
